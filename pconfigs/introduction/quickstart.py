...
# Trainer class
@pconfiged(runnable=True)  # This class will have a main().
class Trainer:
...
@pconfig(constructs=Trainer)  # Can use this config to construct Trainer.
class TrainerConfig:
...
config_1 = TrainerConfig(  # Set just the learning rate. Use
    lr=1e-3,  #  ..defaults for other config parameters.
)

config_2 = TrainerConfig(
    config_1,  # Copy all parameters from the base experiment.
